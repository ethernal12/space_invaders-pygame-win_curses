...
class Test_Vesolje(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.vesolje = Vesolje(st_vrst=self.st_vrst,
    ...
    def test___init__(self):
        self.assertEqual(self.vesolje.st_vrst, self.st_vrst)
        ...
        self.assertEqual(len(self.vesolje.stev_vesoljcev), self.st_vrst * self.st_vesoljcev)
    ...
    def test_menjava_smeri_vesoljcev(self):
        self.vesolje.stev_vesoljcev[-1].x = 1
        self.vesolje.menjava_smeri_vesoljcev()
        for i in range(len(self.vesolje.stev_vesoljcev)):
            self.assertEqual(self.vesolje.stev_vesoljcev[i].smer, 'levo')

        self.vesolje.stev_vesoljcev[-1].x = 0
        self.vesolje.menjava_smeri_vesoljcev()
        for i in range(len(self.vesolje.stev_vesoljcev)):
